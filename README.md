...
### 导入依赖
`implementation 'com.github.WDCloudEdu:WDAndroidAnalytics:1.0'  `
...
### 4.统计初始化
    ...
    WdAnalytics.init(Context this, String "人人通", Boolen true, int 0,String baseUrl);
    
    参数说明：
    当前上下文，项目名称，是否开启日志，上传时间间隔（分钟）默认为5分钟，设置域名（默认为西藏地址，如需更换请传入对应域名）
    
`注意：最新版本新增手动设置access-key，必须在调用统计前设置，否则会使用默认access-key  （access-key 是后台提供参数）`

`WdAnalytics.setWdAcceesKey(String accesskey);`
...
### 8.使用时长统计
    ...
    在App退出前调用setExitApp（）   最新版本改动：setExitApp（Context context）;
...
`Maven`
    ...
    1.1.0  新增手动设置Access-key入口
    
    1.1.2  Android 8.0 服务优化  （最新）
`jitpack`
    
    1.0.1 统计参数调整
    
    1.0.2 新增手动设置Access-key 入口 Android 8.0服务优化 （最新）
